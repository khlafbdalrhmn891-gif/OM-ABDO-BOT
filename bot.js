...
// مع MongoDB Atlas للتخزين الدائم
// ==============================
...
// ====== اتصال MongoDB ======
// ضع رابط MongoDB Atlas هنا
const MONGO_URI = process.env.MONGO_URI || 'mongodb+srv://USERNAME:PASSWORD@cluster0.xxxxx.mongodb.net/om-abdo-bot?retryWrites=true&w=majority';
...
// تحميل الإعدادات من DB عند التشغيل
async function loadSettings() {
  try {
    ...
    const oc = await Settings.findOne({ key: 'orderCounter' });
    if (oc) orderCounter = oc.value;
    else {
      // ابدأ من آخر أوردر موجود
      const last = await Order.findOne().sort({ createdAt: -1 });
      if (last) orderCounter = parseInt(last.orderId.replace('ORD-','')) || 1000;
    }
    console.log('✅ الإعدادات اتحملت | نظافة:', cleaningFee, '| استقبال:', ordersOpen, '| آخر أوردر:', orderCounter);
  } catch(e) {
    console.error('⚠️ خطأ في تحميل الإعدادات:', e.message);
  }
}
...
function mainMenu() {
  ...
  return `🌾 ${STORE_NAME} ترحب بكم 🌾\n\nاختار من القائمة:\n\n1️⃣ عرض المنتجات\n2️⃣ إضافة للسلة\n3️⃣ سلتي وتعديلها\n4️⃣ تتبع طلب\n0️⃣ الرئيسية\n\n💳 الدفع: فودافون كاش ${VODAFONE}\n📞 للتواصل: ${ADMIN_PHONE}`;
}

function prodList() {
  let t = '🌿 *منتجاتنا:*\n\n';
  products.forEach((p, i) => {
    const priceStr = p.price > 0 ? `${p.price} ج/${p.unit}` : 'سعر متغير ⚠️';
    t += `${p.emoji} *${i+1}. ${p.name}* - ${priceStr}\n`;
  });
  return t;
}
...
function cartText(cart) {
  ...
  cart.forEach(i => {
    ...
    const priceStr = s > 0 ? `${s} ج` : 'سعر متغير';
    t += `${i.emoji} ${i.name} (${qStr}) = ${priceStr}\n`;
...
// ====== حفظ الأوردر في MongoDB ======
async function saveOrder(sess, id, total) {
  try {
    ...
    console.log(`✅ أوردر ${id} اتحفظ في MongoDB`);
    return true;
  } catch(e) {
    console.error('❌ خطأ في حفظ الأوردر:', e.message);
    return false;
  }
}
...
client.on('message', async (msg) => {
  ...
  if (sess.step === 'main') {
    ...
    else if (t==='4') { sess.step='track'; await msg.reply('🔍 أرسل رقم الأوردر:\n0️⃣ رجوع'); }
  ...
  if (sess.step === 'cart_menu') {
    if (t==='1') { sess.step='choose_product'; await msg.reply(prodList() + '\n─────────────\nأرسل رقم المنتج:\n0️⃣ رجوع'); }
    else if (t==='2') {
      if (!sess.cart.length) { await msg.reply('❌ السلة فارغة!\n0️⃣ رجوع'); return; }
      let list = '🗑️ اختار رقم الصنف اللي عايز تشيله:\n\n';
      sess.cart.forEach((item, i) => {
        const qStr = item.askWeight ? `${item.qty} فرخة/${item.weight} كيلو` : `${item.qty} ${item.unit}`;
        list += `${i+1}. ${item.emoji} ${item.name} (${qStr})\n`;
      });
      list += '\n0️⃣ رجوع بدون حذف';
  ...
  if (sess.step === 'choose_product') {
    ...
    if (isNaN(n) || !products[n]) { await msg.reply(`❌ رقم غير صحيح! من 1 لـ ${products.length}\n0️⃣ رجوع`); return; }
  ...
  if (sess.step === 'choose_weight') {
    ...
    const p = sess.chosen, cnt = p._count;
  ...
  if (sess.step === 'get_address') {
    ...
    const summary = sess.cart.map(i => {
      ...
      const base  = i.price > 0 ? (i.askWeight ? Math.round(i.price*i.weight) : i.price*i.qty) : 0;
      const clean = i.cleanTotal || 0;
      const pStr  = (base+clean) > 0 ? (base+clean)+' ج' : 'سيتم إرسال السعر';
      let line = `${i.emoji} ${i.name} (${qStr}) = ${pStr}`;
      if (i.wantClean && clean > 0) line += `\n   🧹 ${i.qty} × ${i.cleanFee} = ${clean} ج`;
      return line;
    }).join('\n');

    // حفظ في MongoDB
    const saved = await saveOrder(sess, id, total);
    const payMsg = total > 0 ? `\n─────────────\n💳 الدفع: فودافون كاش\n📱 ${VODAFONE}\nالمبلغ: ${total} ج\n\nأو الدفع عند الاستلام` : '';
    const statusMsg = saved ? '' : '\n⚠️ تعذر الحفظ - تواصل معنا';

    await msg.reply(`✅ *تم تسجيل طلبك!*\n\n🆔 ${id}\n👤 ${sess.name}\n📱 ${sess.phone}\n📍 ${sess.address}\n\n📋 تفاصيل:\n${summary}\n\n💰 الإجمالي: ${total > 0 ? total+' ج' : 'سيتم التواصل'}${payMsg}\n\n🌾 شكراً لتعاملك مع ${STORE_NAME}\nللاستفسار: ${ADMIN_PHONE}${statusMsg}`);
    sess.cart = []; sess.step = 'main';
    return;
  }
